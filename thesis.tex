...
\begin{document}
    ...
    \section{実験と考察} % セクション．実験
        ...
        3節では，2節で識別に失敗した事例について，正確に識別を行うための改善案を示す．
    ...
    \subsection{改善案}
        \ref{identify_result}節で正常に識別できなかった事例2と事例3に対し，識別精度を改善するための手法を提案する．
        \subsubsection{事例2}
            a
        \subsubsection{事例3}
            反射光が写り込んでいる盤面画像に対し，白石のしきい値を上げることで反射光の誤った識別を減らせると考えた．
            そこで，事例3の画像に対し，\label{threshold}章で行ったように，白石のしきい値を変動させた際の感度と特異度を求め，事例3に最適なしきい値の検討を行った．
            %! グラフと，誤った識別をしてた場所が正常に識別できた図を
